	...
	if(texto_linha[indice] == '/' && texto_linha[indice+1] == '*'){
		...
		while(getline(arq,texto_linha)){
            indice = nao_eh_vazio_indice(texto_linha.c_str());	
                if(indice != -1){
                	if(!(texto_linha[indice+1] == '/' && texto_linha[indice] == '*')){
                		linha_comments++;
          				//cout << "Comentario detectado " << texto_linha <<endl;
					} else if (texto_linha[indice+1] == '/' && texto_linha[indice] == '*') {
					    linha_comments++;
          				//cout << "Comentario detectado " << texto_linha <<endl;
						break;
					}
...
int conta_final(ifstream& arq) {
    ...
    cout << "Total de linhas uteis : "  << val_total -(val_branco + val_comment) << endl;
